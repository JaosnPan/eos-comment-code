...
class privileged_api : public context_aware_api {
   public:
      ...
      /**
       * update the resource limits associated with an account[更新与帐户关联的资源限制].  Note these new values will not take effect until the
       * next resource "tick" which is currently defined as a cycle boundary inside a block[请注意，这些新值在生效之前不会生效下一个资源“tick”，当前被定义为块内的循环边界].
       *
       * @param account - the account whose limits are being modified[正在修改其限制的帐户]
       * @param ram_bytes - the limit for ram bytes[ram字节的限制]
       * @param net_weight - the weight for determining share of network capacity[确定网络容量份额的权重]
       * @param cpu_weight - the weight for determining share of compute capacity[确定计算容量份额的权重]
       */
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         ...
         /** 调用资源限制管理器的set_account_limits接口设置账户的资源限制，如果是新增资源（set_account_limits返回ture），则需要在validate_ram_usage map对象中增加账户名 */
