...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         /** 实例化genesis_state，如果编译时指定的创世公钥与代码不符，则抛出异常 */
      ...
      /** 获取默认的控制配置存储到chain_config中 */
      my->chain_config = controller::config();

      /** 将从配置文件中读取的账户白/黑名单、合约白/黑名单添加到选项器 */
      ...
      /** 如果存在action黑名单项则检查每一列并添加到chain_config->action_blacklist中 */
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            /** 遍历选项，每一项格式为[账户::action],首先检查每一项格式,格式错误则抛出异常 */
            auto pos = a.find( "::" );
            EOS_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
            /** 将账户名和action名唯一的追加到chain_config->action_blacklist，避免重复的项 */
      ...
      /** 如果存在密钥黑名单项则检查每一列并添加到chain_config->key_blacklist中 */
      ...
      /** 如果指定了块的绝对目录路径则直接赋值，否则的放在data_dir下的blocks-dir相对路径中 */
      ...
      /** 如果指定了作为检查点执行的[BLOCK_NUM,BLOCK_ID]对，则需要唯一的追加到已加载的检查点执行map中 */
      if( options.count("checkpoint") ) {
         auto cps = options.at("checkpoint").as<vector<string>>();
         /** 扩展loaded_checkpoints的大小为cps.size() */
         my->loaded_checkpoints.reserve(cps.size());
         for( const auto& cp : cps ) {
            /** 按照json方式解析每一个检查点对 */
            auto item = fc::json::from_string(cp).as<std::pair<uint32_t,block_id_type>>();
            /** 查找是否已经追加了该检查点，如果追加了则抛出异常，否则追加 */
      ...
      /** 更新不要跳过重放不可逆块时可以跳过的任何检查，注意这个配置是一个命令行配置 */
      my->chain_config->force_all_checks = options.at( "force-all-checks" ).as<bool>();
      /** 更新是否在控制台输出合约的输出信息 */
      my->chain_config->contracts_console = options.at( "contracts-console" ).as<bool>();
      /** 如果需要导出配置文件或者打印配置文件，从块的存储路径下的blocks.log中提取genesis_state并以JSON格式记录，写入指定文件，然后退出 */
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         genesis_state gs;
         
         /** 如果在块存储路径下存在blocks.log文件，则使用extract_genesis_state接口获取genesis_state对象，否则报错 */
         ...
         /** 如果是打印创世配置文件，则直接通过to_pretty_string接口将genesis_state对象格式化成json格式输出到控制台上 */
         if( options.at( "print-genesis-json" ).as<bool>()) {
            ilog( "Genesis JSON:\n${genesis}", ("genesis", json::to_pretty_string( gs )));
         }

         /** 如果需要存储创世配置文件则字节调用save_to_file将genesis_state对象格式化存储到指定的文件中 */
      ...
      /** 如果需要导出将可逆数据块到指定的文件夹则将可逆块数据库以可移植格式导出到指定文件然后退出 */
      ...
      /** 如果需要删除所有区块，那么删除block和state文件夹下的所有内容 */
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         /** 如果需要硬重放链，那么清除链状态数据库，从块日志中恢复尽可能多的块，然后重放这些块 */
         ilog( "Hard replay requested: deleting state database" );
         fc::remove_all( my->chain_config->state_dir );

         /** 首先修复log, truncate-at-block意为外部指定一个块号，此块编号处停止硬重放/阻止日志恢复（如果设置为非零数字），默认为0    */
         auto backup_dir = block_log::repair_log( my->blocks_dir, options.at( "truncate-at-block" ).as<uint32_t>());
         /** 如果reversible目录存在或者需要fix-reversible-blocks, 那么需要恢复可逆的块 */
      ...
      /** 如果外部指定了genesis文件，则需要更新程序中的genesis状态对象 */
      if( options.count( "genesis-json" )) {
         ...
         /** 如果指定了创世时间戳，那么意味着区块链必须是一条干净的链，不能存在blocks.log，否则提示用户在一条干净的链上执行 */
         EOS_ASSERT( !fc::exists( my->blocks_dir / "blocks.log" ),
                     plugin_config_exception,
                    "Genesis state can only be set on a fresh blockchain." );
      ...
      if ( options.count("read-mode") ) {
         /**如果指定了读模式，那么必须不能为不可逆模式，目前不可逆模式不支持 */
      ...
      /** 更新配置，注意emplace的用法，对比boost库的optional，此方法可以避免构造后赋值的开销，直接就地创建对象 */
      my->chain.emplace( *my->chain_config );
      my->chain_id.emplace( my->chain->get_chain_id());

      // set up method providers
      /**
      * JASON_TO_DO_FUTURE:目前关于这块怎么注册还是没看明白，后续理解深刻后补充
      */
      /** 注册通过块号获取块的方法提供者为fetch_block_by_number */
      ...
      /** 注册通过块ID获取块的方法提供者为fetch_block_by_id */
      /** TIPS:，区块id是一个与区块内容有关的标识，而区块序号仅仅是从0计数的区块编号
      * id实际是由两部分组成：对区块内容做Hash运算，然后与区块序号进行拼接而得到的,
      * 所以id实际包含了区块序号，通过id可以计算得出序号 
      */
      ...
      /** 注册获取最后一个不可逆块方法提供者为last_irreversible_block_num */
...
/**
 * @brief 导出
 * @param[in] reversible_dir:   可逆块存储路径
 * @param[in] reversible_blocks_file:   可逆块导出路径
 * @return true、false
*/
