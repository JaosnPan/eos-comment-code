...
namespace eosio {
   ...
   static constexpr uint64_t name_suffix( uint64_t n ) {
      ...
      uint32_t tmp = 0;
      for( int32_t remaining_bits = 59; remaining_bits >= 4; remaining_bits -= 5 ) { // Note: remaining_bits must remain signed integer[remaining_bits必须保持有符号整数]
         // Get characters one-by-one in name in order from left to right (not including the 13th character)[从左到右依次取字符(不包括第13个字符)]
         auto c = (n >> remaining_bits) & 0x1Full;
         if( !c ) { // if this character is a dot[如果这个字符是点]
            tmp = static_cast<uint32_t>(remaining_bits);
         } else { // if this character is not a dot[如果这个字符不是点]
            remaining_bits_after_last_actual_dot = tmp;
         }
      }

      uint64_t thirteenth_character = n & 0x0Full;
      if( thirteenth_character ) { // if 13th character is not a dot[如果第十三个字符不是点]
         remaining_bits_after_last_actual_dot = tmp;
      }

      if( remaining_bits_after_last_actual_dot == 0 ) // there is no actual dot in the name other than potentially leading dots[除了潜在的前导点之外，名字中没有实际的点]
         return n;

      // At this point remaining_bits_after_last_actual_dot has to be within the range of 4 to 59 (and restricted to increments of 5)[在这一点上，remaining_bits_after_last_actual_dot必须在4到59之间(并且限制了5的增量)。].

      // Mask for remaining bits corresponding to characters after last actual dot, except for 4 least significant bits (corresponds to 13th character)
      // [除4个最小有效位(对应第13个字符)外，与最后一个实际点后面的字符对应的剩余位的掩码].
