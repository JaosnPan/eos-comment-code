...
extern "C" {
    ...
    */
    ...
    /**
     *  Cancels a deferred transaction[取消延期交易].
     *
     *  @brief Cancels a deferred transaction[取消延期交易].
     *  @param sender_id - The id of the senderp[发送者的ID]
     *
     *  @pre The deferred transaction ID exists[存在延迟交易的ID].
     *  @pre The deferred transaction ID has not yet been published[延迟交易ID尚未发布].
     *  @post Deferred transaction canceled[延迟交易已取消].
     *
     *  @return 1 if transaction was canceled, 0 if transaction was not found[如果未找交易返回1，否则返回0]
     *
     *  Example:
*
     *  @code
